...
export default function ResourcesPage() {
  ...
  const filteredResources = resources.filter(r => {
    const matchesSearch = r.courseName.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <>
          ...
          <Stack gap="lg">
            <Card radius="lg" padding="xl" withBorder>
              ...
              <Stack gap="xs" style={{ position: 'relative', zIndex: 1 }}>
                <Title order={1} size="h2">Resource Library</Title>
                <Text c="dimmed" maw={600}>
                  Access a comprehensive collection of course materials, notes, and previous year questions. 
                  Organized by year and subject for easy access.
                </Text>
              </Stack>
            </Card>
            ...
            {loading ? (
              ...
              <>
                  {activeTab && DRIVE_LINKS[activeTab] && (
                      <Card withBorder radius="md" padding="md" mb="lg" bg="blue.0" style={{ borderColor: 'var(--mantine-color-blue-2)' }}>
                          <Group justify="space-between">
                              <Group>
                                  ...
                                  <div>
                                      <Text fw={600} size="sm">Incomplete Materials?</Text>
                                      <Text size="xs" c="dimmed">
                                          Access the full Google Drive folder for {
                                              activeTab === 'all' ? 'all years' : 
                                              activeTab === '2,3' ? '2nd & 3rd Years' :
                                              `Year ${activeTab}`
                                          }.
                                      </Text>
                                  </div>
                  ...
                  {filteredResources.length === 0 ? (
                      <Center h={200}>
                          <Stack align="center" gap="xs">
                              ...
                              <Text c="dimmed">No resources found matching your criteria.</Text>
                          </Stack>
                      </Center>
                  ) : (
                      <Accordion variant="separated" radius="md">
                          {filteredResources.map(subject => (
                              <Accordion.Item key={subject._id} value={subject.courseCode}>
                                  <Accordion.Control icon={<IconBook size={20} color="var(--mantine-color-blue-6)" />}>
                                      <Group justify="space-between" w="100%" pr="md">
                                        <div>
                                            ...
                                            <Text size="xs" c="dimmed">{subject.courseCode} • Year {subject.year}</Text>
                                        </div>
                                          ...
                                          {subject.syllabus && (
                                              <div>
                                                  <Group gap="xs" mb="xs">
                                                      ...
                                                      <Text size="sm" fw={600} tt="uppercase" c="dimmed">Syllabus</Text>
                                                  </Group>
                                                  <SimpleGrid cols={{ base: 1, sm: 2, md: 3, lg: 4 }} spacing="md" verticalSpacing="md">
                                                      <ResourceItemCard 
                                                          title="Syllabus" 
                                                          type="SYLLABUS" 
                                                          onClick={() => handlePreview(subject.syllabus!.linkUrl, `${subject.courseName} Syllabus`)} 
                                                      />
                                                  </SimpleGrid>
                                              </div>
                                          )}
                                          ...
                                          {subject.modules.length > 0 && (
                                              <div>
                                                  <Group gap="xs" mb="xs">
                                                      ...
                                                      <Text size="sm" fw={600} tt="uppercase" c="dimmed">Notes</Text>
                                                  </Group>
                                                  <SimpleGrid cols={{ base: 1, sm: 2, md: 3, lg: 4 }} spacing="md" verticalSpacing="md">
                                                      {subject.modules.map((mod, idx) => (
                                                          <ResourceItemCard 
                                                              ...
                                                              type="NOTES"
                                                              onClick={() => handlePreview(mod.linkUrl, mod.title)}
                                                          />
                                                      ))}
                                                  </SimpleGrid>
                                              </div>
                                          ...
                                          {subject.pyqs.length > 0 && (
                                              <div>
                                                  <Group gap="xs" mb="xs">
                                                      ...
                                                      <Text size="sm" fw={600} tt="uppercase" c="dimmed">Previous Year Questions (PYQ)</Text>
                                                  </Group>
                                                  <SimpleGrid cols={{ base: 1, sm: 2, md: 3, lg: 4 }} spacing="md" verticalSpacing="md">
                                                      {subject.pyqs.map((pyq, idx) => (
                                                          <ResourceItemCard 
                                                              ...
                                                              type="PYQ"
                                                              onClick={() => handlePreview(pyq.linkUrl, `${pyq.exam} ${pyq.year}`)}
                                                          />
                                                      ))}
                                                  </SimpleGrid>
                                              </div>
      ...
      <Modal 
        ...
        onClose={() => { closePreview(); setPreviewUrl(''); }}
        title={previewTitle}
        size="xl"
        ...
        {previewUrl && (
            <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column' }}>
                <iframe 
                    ...
                    style={{ width: '100%', flex: 1, border: 'none', borderRadius: '8px', backgroundColor: '#f1f3f5' }} 
                    title={previewTitle}
                />
    ...
    return (
        ...
        >
            <Group wrap="nowrap" align="start">
                ...
                >
                    {isPyq ? <IconFolderOpen size={18} /> : isSyllabus ? <IconFileText size={18} /> : <IconBook size={18} />}
                </ThemeIcon>
                
                <div style={{ flex: 1, minWidth: 0 }}>
                    <Text size="sm" fw={600} lineClamp={2} title={title} style={{ lineHeight: 1.4 }}>
                        {title}
                    </Text>
                    <Group gap={6} mt={4}>
                        <Badge size="xs" variant="dot" color={isPyq ? 'orange' : isSyllabus ? 'grape' : 'blue'}>
                            {type}
                        </Badge>
                    </Group>
                </div>
