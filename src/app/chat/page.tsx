...
function ChatPageContent() {
  ...
  const pinnedCount = useMemo(
    () => recentDms.filter((dm) => dm.isPinned).length,
    [recentDms]
  );
  ...
  const sidebarWidth = useMemo(
    () => (isMobile ? "100%" : isTablet ? 260 : 320),
    [isMobile, isTablet]
  );
  const sidebarMaxWidth = useMemo(
    () => (isMobile ? "100%" : isTablet ? 300 : 360),
    [isMobile, isTablet]
  );
  ...
  const handleSearchUsers = async (query: string) => {
    ...
    try {
      const res = await fetch(`/api/users?search=${query}&limit=5`, {
  ...
  const fetchConversations = useCallback(
    async (options?: { showSpinner?: boolean }) => {
      ...
      const shouldShowSpinner =
        options?.showSpinner || recentDmsRef.current.length === 0;
      if (shouldShowSpinner) setIsConversationsLoading(true);
      try {
        const res = await fetch(
          `/api/chat?type=conversations&userId=${profile._id}`,
          { headers: getAuthHeaders() }
        );
        if (!res.ok) return;
        const data = await res.json();
        ...
        const hash = incoming
          .map(
            (dm: any) =>
              `${dm._id}:${dm.unreadCount ?? 0}:${dm.lastMessageAt ?? ""}:${
                dm.isPinned ? 1 : 0
              }`
          )
          .join("|");
  ...
  const handleConversationAction = useCallback(
    async (targetId: string, action: "pin" | "unpin" | "delete") => {
      if (!profile?._id) return;
      if (action === "pin" && pinnedCount >= 3) {
        showError(
          { message: "You can pin up to 3 conversations." },
          "Pin Limit"
        );
        return;
      }
      if (
        action === "delete" &&
        !confirm("Delete this conversation for both participants?")
      ) {
        return;
      }
  ...
  useEffect(() => {
    if (dmRecipientId) {
      ...
      (async () => {
        try {
          const res = await fetch(`/api/users/${dmRecipientId}`, {
            headers: getAuthHeaders(),
          });
          const data = await res.json();
  ...
  const fetchMessages = useCallback(
    async (options?: { showSpinner?: boolean }) => {
      ...
      const shouldShowSpinner =
        options?.showSpinner || messagesRef.current.length === 0;
      ...
      try {
        ...
        if (type === "dm" && dmRecipientId)
          query.append("recipientId", dmRecipientId);
        ...
        if (res.ok) {
          ...
          const payloadHash = incomingMessages
            .map((msg: Message) => `${msg._id}:${msg.createdAt}`)
            .join("|");
          if (
            messagesHashRef.current === payloadHash &&
            messagesRef.current.length === incomingMessages.length
          ) {
            return;
          }
          messagesHashRef.current = payloadHash;
          ...
          const shouldScroll =
            incomingMessages.length > prev.length && autoScrollRef.current;
  ...
  useEffect(() => {
    ...
    if (!dmSnapshotReady.current) {
      dmUnreadSnapshot.current = recentDms.reduce<Record<string, number>>(
        (acc, dm) => {
    ...
    recentDms.forEach((dm) => {
      ...
      if (current > previous) {
        const conversationActive =
          activeTab === "dm" && dmRecipientId === dm._id;
        if (!conversationActive) {
          const delta = current - previous;
          showInfo(
            `${dm.name} sent ${delta} new message${delta > 1 ? "s" : ""}`
          );
        }
    ...
    return (
      <>
        ...
        <Container size="sm" py="xl" ta="center">
          ...
          <Text mt="md">
            This chat is restricted to VIT students only (@vitstudent.ac.in).
          </Text>
        </Container>
      </>
    );
  ...
  return (
    <>
      ...
      <Container
        size="xl"
        py={isMobile ? 0 : "xl"}
        h={isMobile ? "calc(100dvh - 60px)" : "calc(100vh - 80px)"}
        px={isMobile ? 0 : "md"}
      >
        <Paper
          withBorder={!isMobile}
            ...
            >
              ...
              >
                ...
                <Group justify="space-between" mb="xs">
                  <Text fw={700} c="dimmed" size="xs" tt="uppercase">
                    Channels
                  </Text>
                  <Tooltip label="Refresh Messages">
                    <ActionIcon
                      variant="subtle"
                      color="gray"
                      onClick={() => fetchMessages({ showSpinner: true })}
                      size="xs"
                    >
                      <IconRefresh size={14} />
                    </ActionIcon>
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              {dm.isPinned && (
                                <IconStarFilled size={12} color="#ffd43b" />
                              )}
                            </Group>
                          </Group>
                          <Group
                ...
                {dmRecipientId &&
                  !recentDms.find((d) => d._id === dmRecipientId) && (
                    <Button
                      variant="filled"
                      color="blue"
                      justify="flex-start"
            ...
            >
                    ...
                    {activeTab === "dm" && dmRecipientId && (
                      <Menu shadow="md" width={200}>
                        ...
                        <Menu.Dropdown>
                          <Menu.Item
                            color={
                              blockedUsers.includes(dmRecipientId)
                                ? "green"
                                : "red"
                            }
              ...
              >
                {replyingTo && (
                  ...
                  >
                    <Box>
                      <Text size="xs" fw={700} c={isDark ? "white" : "black"}>
                ...
                <Group gap="xs" align="center" wrap="nowrap">
                  ...
                  <TextInput
                    placeholder={`Message ${
                      activeTab === "dm" ? dmUser?.name || "User" : activeTab
                    }...`}
                    value={newMessage}
