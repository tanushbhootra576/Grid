...
export default function ProfilePage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!profile?.profileLocked && formData.branch && formData.year) {
      if (
        !window.confirm(
          "Setting your Branch and Year will lock your profile. You will not be able to change them later. Are you sure you want to proceed?"
        )
      ) {
        return;
      }
      ...
      const load = async () => {
        ...
        try {
          ...
          const rRes = await fetch(
            `/api/resources?uploaderId=${pid}&includePending=true`,
            { headers: getAuthHeaders() }
          );
    ...
    try {
      ...
      if (res.ok) {
        setMySkills((prev) =>
          prev.map((s) =>
            s._id === skillId
              ? { ...s, status: newStatus as "OPEN" | "CLOSED" }
              : s
          )
        );
        showSuccess(
          `Skill marked as ${newStatus === "CLOSED" ? "completed" : "open"}`
        );
      }
    } catch {
  ...
  const openEditProject = (project: Project) => {
    ...
    if (
      profile?._id &&
      !members.some((m) => String(m._id) === String(profile._id))
    ) {
    ...
    try {
      const res = await fetch(`/api/projects/${editProjectModal.project._id}`, {
        ...
        body: JSON.stringify({
          ...
          techStack: String(editProjectData.techStack)
            .split(",")
            .map((t: string) => t.trim())
            .filter(Boolean),
    ...
    const handle = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/users?search=${encodeURIComponent(q)}&limit=6`,
          { headers: getAuthHeaders() }
        );
        const data = await res.json();
        if (res.ok && Array.isArray(data.users)) {
          ...
          setContributorResults(
            data.users.filter(
              (u: UserResult) =>
                !contributors.some((c) => String(c._id) === String(u._id))
            )
          );
        }
      } catch (e) {
  ...
  const addContributor = (u: UserResult) => {
    ...
    setContributors((prev) =>
      prev.some((c) => String(c._id) === id) ? prev : [...prev, u]
    );
  ...
  const handleDelete = async () => {
    ...
    const performDelete = async () => {
      ...
      if (!res.ok && res.status !== 404) {
        console.error(
          "Failed to delete user data from DB after Firebase deletion"
        );
      }
    ...
    try {
      ...
      if (err.code === "auth/requires-recent-login") {
        showError(
          {
            message:
              "Security check: Please sign in again to confirm deletion.",
          },
          "Delete Failed"
        );
        try {
    ...
    <>
      ...
      <Container size="md" py="xl">
        ...
        <Paper withBorder shadow="sm" p="xl" radius="md" pos="relative">
          ...
          <Group mb="xl">
            <Avatar src={user.photoURL} size={100} radius={100} color="blue">
              {formData.name?.[0]}
            </Avatar>
            <div>
          ...
          <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="lg">
            ...
            <Group grow>
              ...
              <NumberInput
                ...
                value={formData.year}
                onChange={(val) =>
                  setFormData({ ...formData, year: Number(val) })
                }
                disabled={profile?.profileLocked}
              />
            </Group>
              ...
              <Card key={p._id} withBorder padding="md" radius="md" shadow="xs">
                ...
                <Group mt="xs" gap={4} wrap="wrap">
                  {(p.techStack || []).slice(0, 6).map((t: string) => (
                    <Badge key={t} color="blue" variant="light" size="sm">
                      {t}
                    </Badge>
                  ))}
                </Group>
          ...
          <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
            {mySkills.map((s) => (
              <Card
                ...
                radius="md"
                shadow="xs"
                style={{ opacity: s.status === "CLOSED" ? 0.7 : 1 }}
              >
                <Group justify="space-between" mb={4}>
                  <Text
                    fw={600}
                    td={s.status === "CLOSED" ? "line-through" : undefined}
                  >
                    {s.title}
                  </Text>
                  <Group gap={4}>
                    <Badge color={s.type === "OFFER" ? "blue" : "orange"}>
                      {s.type}
                    </Badge>
                ...
                <Group justify="space-between" mt="md">
                  <Group gap={4} wrap="wrap">
                    {(s.tags || []).slice(0, 3).map((t: string) => (
                      <Badge key={t} color="violet" variant="light" size="sm">
                        {t}
                      </Badge>
                    ))}
                  </Group>
                  <Group gap={4}>
                    <Tooltip
                      label={
                        s.status === "OPEN" ? "Mark as Completed" : "Re-open"
                      }
                    >
                      <ActionIcon
      ...
      >
        <TextInput
          ...
          onChange={(e) =>
            setEditProjectData((d) => ({ ...d, title: e.target.value }))
          }
        />
        <Textarea
          label="Description"
          ...
          onChange={(e) =>
            setEditProjectData((d) => ({ ...d, description: e.target.value }))
          }
        />
        <TextInput
          ...
          onChange={(e) =>
            setEditProjectData((d) => ({ ...d, techStack: e.target.value }))
          }
        />
        <TextInput
          label="Demo Link"
          mb="md"
          value={editProjectData.demoLink}
          onChange={(e) =>
            setEditProjectData((d) => ({ ...d, demoLink: e.target.value }))
          }
        />
        <TextInput
          label="Repo Link"
          mb="md"
          value={editProjectData.repoLink}
          onChange={(e) =>
            setEditProjectData((d) => ({ ...d, repoLink: e.target.value }))
          }
        />
        ...
        <Group gap={6} mb="sm" wrap="wrap">
          {contributors.map((c: Contributor) => {
            ...
            const display =
              c.name ||
              (id === String(profile?._id) ? "You" : id.substring(0, 6));
            return (
              <Badge
                key={id}
                color={id === String(profile?._id) ? "teal" : "blue"}
                rightSection={
        ...
        <Group justify="flex-end" mt="md">
          <Button
            variant="default"
            onClick={() => setEditProjectModal({ open: false, project: null })}
          >
            Cancel
          </Button>
          <Button onClick={submitProjectEdit} loading={loading}>
            Save
          </Button>
        </Group>
      </Modal>
